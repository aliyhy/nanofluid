...
void Foam::phaseSystem::calcKappa()
{
    kappa_ = kappa()();
}
void Foam::phaseSystem::calcCp()
{
    Cp_ = Cp()();
}
...
) const
{
    ...
    (
        iter()() * iter()->Cp(mesh, p, T, patchI)
    );

    for (++iter; iter != phaseModels_.cend(); ++iter)
    {
        tmpCp.ref() += iter()() * iter()->Cp(mesh, p, T, patchI);
    }
...
) const
{
    ...
    (
        iter()() * iter()->Cv(mesh, p, T, patchI)
    );

    for (++iter; iter != phaseModels_.cend(); ++iter)
    {
        tmpCv.ref() += iter()() * iter()->Cv(mesh, p, T, patchI);
    }
...
) const
{
    ...
    (
        iter()() * iter()->Cpv(mesh, p, T, patchI)
    );

    for (++iter; iter != phaseModels_.cend(); ++iter)
    {
        tmpCpv.ref() += iter()() * iter()->Cpv(mesh, p, T, patchI);
    }
