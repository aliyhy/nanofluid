...
void Foam::mixedUnburntEnthalpyFvPatchScalarField::updateCoeffs()
{
    ...
    refValue() = thermo.heu(thermo.p().mesh(), pw, Tw.refValue(), patchi);
    refGrad() = thermo.Cp(thermo.p().mesh(), pw, Tw, patchi)*Tw.refGrad()
      + patch().deltaCoeffs()*
        (
            thermo.heu(thermo.p().mesh(), pw, Tw, patchi)
          - thermo.heu(thermo.p().mesh(), pw, Tw, patch().faceCells())
        );
