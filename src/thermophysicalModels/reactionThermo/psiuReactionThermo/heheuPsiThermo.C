...
void Foam::heheuPsiThermo<BasicPsiThermo, MixtureType>::calculate()
{
  ...
  const scalarField& hCells = this->he_;
    ...
    forAll(TCells, celli)
    {
        ...
        psiCells[celli] = mixture_.psi(mesh, pCells[celli], TCells[celli]);

        muCells[celli] = mixture_.mu(mesh, pCells[celli], TCells[celli]);
        alphaCells[celli] = mixture_.alphah(mesh, pCells[celli], TCells[celli]);
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
                ...
                phe[facei] = mixture_.HE(mesh, pp[facei], pT[facei]);

                ppsi[facei] = mixture_.psi(mesh, pp[facei], pT[facei]);
                pmu[facei] = mixture_.mu(mesh, pp[facei], pT[facei]);
                palpha[facei] = mixture_.alphah(mesh, pp[facei], pT[facei]);
            }
        }
        else
        {
            forAll(pT, facei)
            {
                ...
                if (this->updateT())
                {
                    pT[facei] = mixture_.THE(phe[facei], mesh, pp[facei], pT[facei]);
                }
                ...
                pTu[facei] =
                   ...
                   .THE(pheu[facei], mesh, pp[facei], pTu[facei]);
            }
        }
    }
}
...
//         );
        ...
        muCells[celli] = mixture_.mu(mu.mesh(),pCells[celli], TCells[celli]);
        alphaCells[celli] = mixture_.alphah(alpha.mesh(),pCells[celli], TCells[celli]);
    ...
    forAll(this->T_.boundaryField(), patchi)
    {
        ...
        if (pT.fixesValue())
        {
            forAll(pT, facei)
            {
                ...
                pmu_[facei] = mixture_.mu(mu.mesh(),pp[facei], pT[facei]);
                palpha_[facei] = mixture_.alphah(alpha.mesh(),pp[facei], pT[facei]);
            }
        }
        else
        {
...
) const
{
    ...
    forAll(Tu, celli)
    {
        heu[celli] = this->cellReactants(cells[celli]).HE(mesh, p[celli], Tu[celli]);
    }
...
) const
{
    ...
    {
        heu[facei] =
            this->patchFaceReactants(patchi, facei).HE(mesh, p[facei], Tu[facei]);
    }
    ...
    forAll(TbBf, patchi)
    {
        ...
        forAll(pTb, facei)
        {
            pTb[facei] =
               ...
               .THE( ph[facei], this->T_.mesh(), pp[facei], pT[facei]);
        }
    }
...
Foam::heheuPsiThermo<BasicPsiThermo, MixtureType>::psiu() const
{
    ...
    {
        psiuCells[celli] =
            this->cellReactants(celli).psi(mesh, pCells[celli], TuCells[celli]);
    }
    ...
    forAll(psiuBf, patchi)
    {
        ...
        forAll(ppsiu, facei)
        {
            ppsiu[facei] =
                this->
                patchFaceReactants(patchi, facei).psi(mesh, pp[facei], pTu[facei]);
        }
    }
...
Foam::heheuPsiThermo<BasicPsiThermo, MixtureType>::psib() const
{
    ...
    {
        psibCells[celli] =
            this->cellProducts(celli).psi( mesh, pCells[celli], TbCells[celli]);
    }
    ...
    forAll(psibBf, patchi)
    {
        ...
        forAll(ppsib, facei)
        {
            ppsib[facei] =
                ...
                (patchi, facei).psi(mesh, pp[facei], pTb[facei]);
        }
    }
