...
) const
{
    ...
    {
        rhoInv += Y_[i][celli]/speciesData_[i].rho(mesh, p, T);
    }

    mixtureVol_ =
        Y_[0][celli]/speciesData_[0].rho(mesh, p, T)/rhoInv*speciesData_[0];

    for (label n=1; n<Y_.size(); n++)
    {
        mixtureVol_ +=
            Y_[n][celli]/speciesData_[n].rho(mesh, p, T)/rhoInv*speciesData_[n];
    }
...
) const
{
    ...
    {
        rhoInv +=
            Y_[i].boundaryField()[patchi][facei]/speciesData_[i].rho(mesh, p, T);
    }

    mixtureVol_ =
        Y_[0].boundaryField()[patchi][facei]/speciesData_[0].rho(mesh, p, T)/rhoInv
      * speciesData_[0];

    for (label n=1; n<Y_.size(); n++)
    {
        mixtureVol_ +=
            Y_[n].boundaryField()[patchi][facei]/speciesData_[n].rho(mesh, p, T)
