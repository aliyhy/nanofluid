...
{
    const scalar kf = R.kf(this->mesh(), p, T, c);
    const scalar kr = R.kr(kf, this->mesh(), p, T, c);
...
) const
{
    ...
    for (label i=0; i<this->c_.size(); i++)
    {
        ...
        cp += this->c_[i]*this->specieThermo_[i].cp(this->mesh(), p, T);
    }
    cp /= rho;
    ...
    for (label i=0; i<this->nSpecie_; i++)
    {
        ...
        const scalar hi = this->specieThermo_[si].ha(this->mesh(), p, T);
        dT += hi*dcdt[i];
    }
    dT /= rho*cp;
...
) const
{
    ...
    forAll(this->reactions_, ri)
    {
        if (!reactionsDisabled_[ri])
        {
            ...
            const scalar kf0 = R.kf(this->mesh(), p, T, this->c_);
            const scalar kr0 = R.kr(kf0, this->mesh(), p, T, this->c_);
