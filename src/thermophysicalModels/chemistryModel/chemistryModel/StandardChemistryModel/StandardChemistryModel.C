...
) const
{
    ...
    for (label i=0; i<nSpecie_; i++)
    {
        cp += c_[i]*specieThermo_[i].cp(this->mesh(), p, T);
    }
    cp /= rho;
    ...
    for (label i = 0; i < nSpecie_; i++)
    {
        const scalar hi = specieThermo_[i].ha(this->mesh(), p, T);
        dT += hi*dcdt[i];
    }
    dT /= rho*cp;
...
) const
{
    ...
    forAll(reactions_, ri)
    {
        ...
        const scalar kf0 = R.kf(this->mesh(), p, T, c_);
        const scalar kr0 = R.kr(kf0, this->mesh(), p, T, c_);
