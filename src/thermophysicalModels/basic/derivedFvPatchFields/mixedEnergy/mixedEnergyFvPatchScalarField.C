...
void Foam::mixedEnergyFvPatchScalarField::updateCoeffs()
{
    ...
    const fvMesh& mesh = thermo.p().mesh();
    ...
    refValue() = thermo.he(mesh, pw, Tw.refValue(), patchi);
    refGrad() =
        thermo.Cpv(mesh, pw, Tw, patchi)*Tw.refGrad()
      + patch().deltaCoeffs()*
        (
            thermo.he(mesh, pw, Tw, patchi)
          - thermo.he(mesh, pw, Tw, patch().faceCells())
        );
