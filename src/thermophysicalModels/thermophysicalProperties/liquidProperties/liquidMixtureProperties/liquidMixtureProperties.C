...
) const
{
    ...
    if (p >= pv(mesh, p, Thi, X))
    {
        return Thi;
    }
    else if (p < pv(mesh, p, Tlo, X))
    {
        WarningInFunction
            << "Pressure below triple point pressure: "
            << "p = " << p << " < Pt = " << pv(mesh, p, Tlo, X) <<  nl << endl;
        return -1;
    }
...
) const
{
    ...
    forAll(Xs, i)
    {
        ...
        Xs[i] = properties_[i].pv(mesh, p, Ti)*Xl[i]/p;
    }
...
) const
{
    ...
    forAll(properties_, i)
    {
        if (X[i] > SMALL)
        {
            ...
            scalar rho = properties_[i].rho(mesh, p, Ti);
...
) const
{
    ...
    forAll(properties_, i)
    {
        if (X[i] > SMALL)
        {
            ...
            scalar Ti = min(TrMax*properties_[i].Tc(), T);
            pv += Yi*properties_[i].pv(mesh, p, Ti);
        }
    }
...
) const
{
    ...
    forAll(properties_, i)
    {
        if (X[i] > SMALL)
        {
            ...
            scalar Ti = min(TrMax*properties_[i].Tc(), T);
            hl += Yi*properties_[i].hl(mesh, p, Ti);
        }
    }
...
) const
{
    ...
    forAll(properties_, i)
    {
        if (X[i] > SMALL)
        {
            ...
            scalar Ti = min(TrMax*properties_[i].Tc(), T);
            Cp += Yi*properties_[i].Cp(mesh, p, Ti);
        }
    }
...
) const
{
    ...
    {
        scalar Ti = min(TrMax*properties_[i].Tc(), T);
        scalar Pvs = properties_[i].pv(mesh, p, Ti);
    ...
    forAll(properties_, i)
    {
        ...
        {
            scalar Ti = min(TrMax*properties_[i].Tc(), T);
            sigma += Xs[i]*properties_[i].sigma(mesh, p, Ti);
        }
    }
...
) const
{
    ...
    forAll(properties_, i)
    {
        ...
        {
            scalar Ti = min(TrMax*properties_[i].Tc(), T);
            mu += X[i]*log(properties_[i].mu(mesh, p, Ti));
        }
    }
...
) const
{
    ...
    forAll(properties_, i)
    {
        ...
        scalar Vi = properties_[i].W()/properties_[i].rho(mesh, p, Ti);
        phii[i] = X[i]*Vi;
        pSum += phii[i];
    }
    ...
    forAll(properties_, i)
    {
        ...
        forAll(properties_, j)
        {
            ...
            scalar Kij =
                2.0
               /(
                    1.0/properties_[i].kappa(mesh,p, Ti)
                  + 1.0/properties_[j].kappa(mesh,p, Tj)
                );
            K += phii[i]*phii[j]*Kij;
        }
    }
...
) const
{
    ...
    forAll(properties_, i)
    {
        ...
        {
            scalar Ti = min(TrMax*properties_[i].Tc(), T);
            Dinv += X[i]/properties_[i].D(mesh, p, Ti);
        }
    }
